...
def check_examination_exists(name: str) -> bool:
    session = SessionLocal()
    exists = session.query(Examination).filter(Examination.name == name).first() is not None
    session.close()
    return exists

def get_available_examinations() -> List[Examination]:
    session = SessionLocal()
    examinations = session.query(Examination).filter(Examination.is_available == True).all()
    session.close()
    return examinations
...
def create_examination(name: str, utilization: int, is_available: bool) -> int:
    session = SessionLocal()
    new_examination = Examination(name=name, utilization=utilization, is_available=is_available)
